...
export type WsServerEvent =
  ...
  | { type: 'signal'; data: { symbol: string; signal: string; edge: number; fair: number } }
  | { type: 'fill'; data: { symbol: string; direction: string; qty: number; price: number; pnl: number } }
  | { type: 'log'; data: { level: 'info' | 'warn' | 'error'; message: string; timestamp: number } }
  | { type: 'error'; data: { message: string } }
  | { type: 'plugins'; data: import('./strategy').StrategyDefinition[] }
